...
export class PackedTransaction extends Struct {
    ...
    static fromSigned(signed: SignedTransaction, compression: CompressionType = 1) {
        ...
        switch (compression) {
            case CompressionType.zlib: {
                // compress data
                packed_trx = Bytes.from(deflate(packed_trx.array))
                packed_context_free_data = Bytes.from(deflate(packed_context_free_data.array))
                break
            }
    ...
    getTransaction(): Transaction {
        switch (Number(this.compression)) {
            ...
            // zlib compressed
            case CompressionType.zlib: {
                const inflated = inflate(this.packed_trx.array)
